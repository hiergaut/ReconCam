...
class Triplet {
  public:
    ...
    bool in(const Point3f & Min, const Point3f & Max) const {
    ...
    double dist(const Point3f & Mean) {
        Point3f point(colors[0].h, colors[1].h, colors[2].h);
        return norm(Mean -point);
    }

    friend std::ostream & operator <<(std::ostream & out, Triplet & triplet) {
        std::vector<Color> colors;
        out << colors[0] << colors[1] << colors[2];
        return out;
    }

    operator std::string() {
        std::stringstream stream;
        stream << std::fixed << std::setprecision(2) << "[" << colors[0].h << ", " <<  colors[1].h << ", " << colors[2].h << "]";
        return stream.str();
    } 
};
